from casadi_model import make_casadi_model_integrator, calculate_control_with_single_shooting_problem_2, calculate_control_with_multiple_shooting_problem_2, test_integrator
import os
import sys
...
TARGET_STATE = [0.05, 0.0, 0.0, 0.0] # x x_dot xi xi_dot
...
ACCEPTABLE_POS_ERROR = 0.001 # meters
...
if __name__ == "__main__":
    ...
    try:
        while current_time < TERMINAL_TIME:
            sol = calculate_control_with_single_shooting_problem_2(init_state=X, init_t=current_time, target_state=TARGET_STATE, init_objective=objective, optimization_duration=MPC_PREDICTION_HORIZON, integration_duration=FIXED_CONTROL_DURATION, model=model, F=model_integrator, ipopt_opts=IPOPT_OPTS)
            ...
            # Break calculations if target_state already reached
            if abs(poses[-1][0] - TARGET_STATE[0]) < ACCEPTABLE_POS_ERROR:
                break
    ...
    filename = f"min_time_sh_{CAPSUBOT_MODEL_TYPE}_dur_{int(MPC_PREDICTION_HORIZON*1000)}_T_{int(TERMINAL_TIME*1000)}_{time_now.strftime('dym_%y_%m_%d_hm_%H_%M')}.pkl"
